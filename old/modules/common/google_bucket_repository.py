...
class GoogleBucketResource:
    ...
    def extract_latest_file(self, list_blobs):
        """Return the filename with the latest date within the given BLOB listing.

        Manages collisions only for the latest date.
    ...
    def get_latest_file(self, resource_info):
        ...
        return self.extract_latest_file(list_blobs)
    ...
    def download_dir(self, dir_to_download, local_dir) -> list[ManifestResource]:
        ...
        for blob in self.get_bucket().list_blobs(prefix=dir_name):
            ...
            list_files.append(self._set_blob_download_manifest_status(blob, download_manifest))
        return list_files
    ...
    def download(self, download_descriptor) -> list[ManifestResource]:
        """Download 'something', it could be an entire folder or just a file.
