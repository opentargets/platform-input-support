...
class Downloads:
    ...
    def prepare_gs_download(self, gs_info, resource):
        ...
        if gs_info['spark']:
            output = os.path.join(
                self.path_root, resource.path, resource.output_spark_dir.replace('{suffix}', gs_info['suffix'])
            )
            create_folder(output)
        else:
            create_folder(os.path.join(self.path_root, resource.path))
            output = os.path.join(
                self.path_root, resource.path, resource.output_filename.replace('{suffix}', gs_info['suffix'])
            )
