...
class Disease(IPlugin):
    ...
    def download_and_convert_file(self, resource, output, riot) -> ManifestResource:
        ...
        try:
            download_manifest.path_destination = self.owl_to_json(
                download_manifest.path_destination, output.staging_dir, resource, riot
            )
        ...
        if download_manifest.status_completion == ManifestStatus.COMPLETED:
            ...
            try:
                download_manifest.path_destination = hpo_phenotypes.run(
                    os.path.join(output.prod_dir, conf.etl.hpo_phenotypes.path, conf.etl.hpo_phenotypes.output_filename)
                )
            except Exception as e:
    ...
    def get_ontology_hpo(self, conf, output, riot) -> ManifestResource:
        ...
        if conversion_manifest.status_completion == ManifestStatus.COMPLETED:
            ...
            try:
                ...
                conversion_manifest.path_destination = os.path.join(
                    output.prod_dir, conf.etl.hpo.path, conf.etl.hpo.output_filename
                )
                try:
    ...
    def get_ontology_mondo(self, conf, output, riot) -> ManifestResource:
        ...
        if conversion_manifest.status_completion == ManifestStatus.COMPLETED:
            ...
            try:
                ...
                conversion_manifest.path_destination = os.path.join(
                    output.prod_dir, conf.etl.mondo.path, conf.etl.mondo.output_filename
                )
                try:
                    ...
                    conversion_manifest.msg_completion = (
                        f"MONDO processed data source, JQ filter '{conf.etl.mondo.owl_jq}'"
                    )
        return conversion_manifest
    ...
    def get_ontology_efo(self, conf, output, riot) -> list[ManifestResource]:
        ...
        static_disease_manifest.path_destination = os.path.join(
        ...
        if conversion_manifest.status_completion == ManifestStatus.COMPLETED:
            ...
            try:
                ...
                try:
                    ...
                    conversion_manifest.path_destination = os.path.join(
                        output.prod_dir, conf.etl.efo.path, conf.etl.efo.output_filename
                    )
                    try:
            ...
            static_disease_manifest.msg_completion = 'FAILED due to an error converting and processing source file'
        return [conversion_manifest, static_disease_manifest]

    def process(self, conf, output, cmd_conf):
        """Disease pipeline step implementation.

        It collects and pre-processes EFO, MONDO, HPO and HPO phenotypes data.
