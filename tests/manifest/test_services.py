...
from platform_input_support.manifest.services import ManifestStatus, get_manifest_service
...
def manifest_config():
    ...
    yield yaml, args
    # teardown
    manifest_json = pathlib.Path(yaml.output_dir).joinpath(pathlib.Path(yaml.config.manifest.file_name))
...
class TestManifestService:
    ...
    @pytest.mark.xfail(reason='Failing due to clashing tests using the sample global ManifestService instance')
    def test_update_manifest_after_run(self, manifest_config: tuple) -> None:
        ...
        new_ms.manifest.steps.get(test_step).status_completion = ManifestStatus.COMPLETED
