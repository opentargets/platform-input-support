...
class DownloadResourceTests(unittest.TestCase):
    def setUp(self):
        ...
        _ = get_manifest_service(
            ...
            configuration=Munch.fromDict({'config': {'manifest': {'file_name': 'manifest.json'}}, 'output_dir': '/tmp'})
        )
    ...
    def test_execute_download(self, urlopen_mock):
        ...
        assert re.search('([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))', download_manifest.path_destination)
