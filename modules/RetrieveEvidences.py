...
from definitions import PIS_OUTPUT_EVIDENCES, PIS_EVIDENCES_STATS_FILE, GOOGLE_STORAGE_URI
    ...
    for entry in yaml_dict.evidences:
        ...
        if bucket is not None:
            ...
            if latest_filename_info["latest_filename"] is not None:
                ...
                logger.info("ERROR: The path={} does not contain any recent file".format(google_resource.get_bucket_name()))
                if not args.skip:
                    raise ValueError("ERROR: The path={} does not contain any recent file".format(google_resource.get_bucket_name()))
        else:
            list_files_downladed_failed[google_resource.get_bucket_name()] = entry.resource
